<template>
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}`">
    ...
    >
      ...
      <el-form-item label="投入物资类型" prop="sale_type">
...
const rules = reactive({
  supplier_name: [{ required: true, message: "请输入厂商名称" }],
  sale_type: [{ required: true, message: "请输入投入物资类型" }],
  sup_linkman: [{ required: true, message: "请输入联系人" }],
  sup_contact: [{ required: true, message: "请输入联系人电话" }],
  contact: [{ required: true, message: "请输入厂商电话" }]
