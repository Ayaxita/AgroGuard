...
// 获取记录列表
export const getSheepList = params => {
...
// 获取档案数据
export const openData = params => {
  const result = http.post(PORT1 + `/xipu/chakan`, params, {
    headers: {
      "Content-Type": "application/json"
    }
  });
  result.then(resp => console.log("返回的档案数据", resp));
  return result;
};

//记录评分
// export const MakeScore = (params = {}) => {
//   return http.post(PORT1 + `/xipu/updatedata`, params);
// };
export const MakeScore = (params = {}) => {
  return http.post(PORT1 + `/xipu/updatedata`, params, {
    headers: {
...
export const MakeAScore = params => {
  return http.post(PORT1 + `/xipu/updatedata`, params);
};

// 下载档案
export const exportxipuInfo = params => {
  return http.download(PORT1 + `/xipu/export`, params);
};

// 新增记录
export const addSheep = params => {
...
// 编辑记录
export const editSheep = params => {
  return http.post(PORT1 + `/basicinfo/edit`, params);
};

// 获取记录档案
export const getSheepFamilyTree = params => {
  return http.post(PORT1 + `/basicinfo/familyTree`, params);
};

// 标记死亡
export const markSheepDeath = params => {
  return http.post(PORT1 + `/basicinfo/markSheepDeath`, params);
};

// 标记售出
export const markSheepSale = params => {
  return http.post(PORT1 + `/basicinfo/markSheepSale`, params);
};

//批量标记淘汰
export const markSheepDieOut = params => {
  const promises = params.map(item => {
    return editSheep(item);
  });
...
// 批量添加记录模板
export const BatchAddSheepTemp = params => {
  return axios({
...
// 批量添加记录
export const BatchAddSheep = params => {
...
// 分区调整
export const sheepTransfer = params => {
  return http.post(PORT1 + `/basicinfo/sheepTransfer`, params);
};
// 添加记录重复字段验证(编号)
export const validateSheepNum = params => {
  return http.post(PORT1 + `/basicinfo/validateSheepNum`, params);
};
// 删除记录
// export const deleteSheep = params => {
