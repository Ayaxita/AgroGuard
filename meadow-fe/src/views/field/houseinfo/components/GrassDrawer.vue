<template>
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}`">
    ...
    >
      ...
      <el-form-item label="监测站点名称" prop="name">
      ...
      <el-form-item label="区域类型" prop="h_type">
        <el-select v-model="drawerProps.row.h_type" clearable>
          <el-option v-for="item in fieldH_typeType" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="草地类型" prop="grass_type">
        <el-select v-model="drawerProps.row.grass_type" clearable>
          <el-option v-for="item in varietyType" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="监测站点功能" prop="function">
        <el-select v-model="drawerProps.row.function" clearable>
          <el-option v-for="item in fieldFuntionType" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="监测站点面积(平方米)" prop="area">
        <el-input v-model="drawerProps.row.area" type="number" step="0.01" clearable></el-input>
      </el-form-item>

      <el-form-item label="区域尺寸" prop="h_lwh">
      ...
      <!-- 地块数量要从基本表算 -->
      <!-- <el-form-item label="地块数量" prop="grass_quantity">
        <el-input v-model="drawerProps.row.grass_quantity" type="number" clearable></el-input>
      </el-form-item> -->
