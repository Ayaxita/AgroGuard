      ...
      >
        ...
        >
          ...
          <el-form-item label="类型" prop="type">
            ...
            >
              <el-option v-for="item in FeedingTypeType" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="草地投入名称" prop="f_name">
            <el-input
          ...
          <el-form-item label="生产厂家" prop="maker_name">
            <el-input
              ...
              :disabled="drawerProps.title === '编辑' || drawerProps.title === '新增'"
            ></el-input>
            <!-- <el-button type="primary" plain @click="openAddWindow" v-if="drawerProps.title === '新增'">单机弹出添加框</el-button>
            <el-button type="primary" plain @click="drawerProps.row.maker_name = null" v-if="drawerProps.title === '新增'">
              取消选择
            </el-button> -->
          </el-form-item>
          ...
          <p style="color: red; text-align: center">出库数量在填入生产厂家，名称和类型后才能填入</p>
          <el-form-item label="出库数量" prop="num">
            <el-input
              ...
              clearable
              :disabled="!(drawerProps.row.maker_name && drawerProps.row.f_name && drawerProps.row.type)"
            ></el-input>
          </el-form-item>

          <el-form-item label="出库仓库" prop="warehouse_num">
            <el-input type="number" v-model="drawerProps.row.warehouse_num" clearable></el-input>
          </el-form-item>

          <el-form-item label="出库用途" prop="out_purposes">
            <el-input type="textarea" v-model="drawerProps.row.out_purposes" clearable></el-input>
          </el-form-item>
          ...
          <el-form-item label="备注信息" prop="notes">
            <el-input type="textarea" v-model="drawerProps.row.notes" clearable></el-input>
          </el-form-item>
          <!-- Assuming f_staff is not excluded -->
...
import { Generate_outbound_no, validate_outNum } from "../api/manu";
...
const validateProps = (rule: any, value: any, callback: any) => {
  ...
  validate_outNum({
    ...
    name: drawerProps.value.row.f_name,
...
const rules = reactive({
  ...
  f_name: [{ required: true, message: "请输入草地投入名称" }],
...
const selectEwe = async selectedList => {
  ...
  try {
    await ElMessageBox.confirm(
      `确认选择厂家为 ${selectedMakername} 并且类型和名称为  ${selectedFname}
    的草地投入嘛?`,
      "温馨提示",
      {
        type: "warning"
    ...
    drawerProps.value.row.f_name = selectedFname;
...
const openAddWindow = () => {
  let newWindow = window.open(
    "http://localhost:" + import.meta.env.VITE_PORT + "/#/supply/f_suppliersinfo/select_page",
    "name",
...
const acceptParams = async (params: DrawerProps) => {
  ...
  drawerProps.value.row.maker_name = "统一厂商";
  if (drawerProps.value.title === "新增") {
    console.log("自动生成单号");
    const generate_outbound_no = await Generate_outbound_no({ param: null, type: "饲料" }); // 这里的type是自定义的，要是更改的话还得更改疫苗和后端对应的部分
...
const Changedelivery_time = async value => {
  ...
  const generate_outbound_no = await Generate_outbound_no({ param: value, type: "饲料" });
  console.log(generate_outbound_no);
  drawerProps.value.row.outbound_no = generate_outbound_no.data;
};
defineExpose({
