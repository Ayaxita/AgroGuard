  ...
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}`">
    ...
    >
      ...
      <el-form-item label="单价" prop="unit_price">
        <el-input v-model="drawerProps.row.unit_price" type="number" step="0.01" clearable disabled></el-input>
      </el-form-item>
      <el-form-item label="数量" prop="weight">
        <el-input v-model="drawerProps.row.weight" type="number" step="0.01" clearable @input="calculateUnitPrice"></el-input>
      </el-form-item>
      <el-form-item label="总价" prop="total_price">
        <el-input
          v-model="drawerProps.row.total_price"
          type="number"
          step="0.01"
          clearable
...
const rules = reactive({
  ...
  weight: [{ required: true, message: "请输入数量" }],
...
const calculateUnitPrice = () => {
  const { weight, total_price } = drawerProps.value.row;
  if (weight && total_price) {
    drawerProps.value.row.unit_price = total_price / weight;
  } else {
