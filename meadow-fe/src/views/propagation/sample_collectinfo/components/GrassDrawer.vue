    ...
    <div class="drawer-container">
      ...
      >
        ...
        >
          ...
          <!-- 批次采样日期 -->
          <el-form-item label="批次采样日期" prop="E_date">
            <el-date-picker
              v-model="drawerProps.row.E_date"
              type="date"
              format="YYYY-MM-DD"
          ...
          <!-- 样本稀释倍数 -->
          <el-form-item label="样本稀释倍数" prop="dilution_ratio">
            <el-input v-model="drawerProps.row.dilution_ratio" type="number" step="0.01" clearable></el-input>
          </el-form-item>

          <!-- 稀释液种类 -->
          <el-form-item label="样本（密度/活力）" prop="diluent_type">
            <!-- <el-input v-model="drawerProps.row.diluent_type" clearable></el-input> -->
            <el-select v-model="drawerProps.row.diluent_type" clearable>
              <el-option v-for="item in DiluentType" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>

          <!-- 是否废弃 -->
          <el-form-item label="是否废弃" prop="disused">
            <el-select v-model="drawerProps.row.disused" clearable>
...
import { getMaleGrass } from "../api/manu";
...
const getTableList = (params: any) => {
  ...
  console.log("getMaleGrass", getMaleGrass(newParams));
  return getMaleGrass(newParams);
};
