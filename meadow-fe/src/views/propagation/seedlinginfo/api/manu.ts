...
export const getManuList = params => {
  const result = http.post(PORT1 + `/seedlinginfo`, params);
...
export const editManu = params => {
  return http.post(PORT1 + `/seedlinginfo/edit_and_tobasic`, params);
};
// 删除记录
export const delManu = params => {
  return http.post(PORT1 + `/seedlinginfo/del`, params);
};
// 导出数据
export const exportGrassInfo = params => {
  return http.download(PORT1 + `/seedlinginfo/export`, params);
};
