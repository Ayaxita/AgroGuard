<template>
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}草地基本信息`">
    ...
    >
      ...
      <el-form-item label="产出量" prop="milk_volume">
        <el-input v-model="drawerProps.row.milk_volume" clearable></el-input>
      </el-form-item>

      <el-form-item label="年收割次数" prop="lamb_num">
        <el-input v-model="drawerProps.row.lamb_num" type="number" clearable></el-input>
      </el-form-item>
...
const rules = reactive({
  basic_id: [{ required: true, message: "请输入草地信息id" }],
  milk_volume: [{ required: true, message: "请输入产出量" }],
