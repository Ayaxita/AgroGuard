<template>
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}草地信息`">
    ...
    >
      ...
      <el-form-item label="草地来源" prop="manu_info_id">
        <el-select v-model="drawerProps.row!.manu_info_id" filterable placeholder="请选择草地来源" style="width: 240px">
          <el-option
      ...
      <el-form-item label="阶段末生物量" prop="wea_weight">
        <el-input
          v-model="drawerProps.row!.wea_weight"
          placeholder="请填写阶段末生物量"
          type="number"
          step="0.01"
          clearable
        ></el-input>
      ...
      <el-form-item label="抗病指数" prop="gene_b">
        <el-select v-model="drawerProps.row!.gene_b" placeholder="请选择抗病指数" clearable>
          <el-option v-for="item in gene_bType" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="环境指数" prop="gene_c">
        <el-select v-model="drawerProps.row!.gene_c" placeholder="请选择环境指数" clearable>
...
const rules = reactive({
  ...
  f_ele_num: [{ validator: validateProps, trigger: "blur" }],
  f_pre_num: [{ validator: validateProps, trigger: "blur" }],
  m_ele_num: [{ validator: validateProps, trigger: "blur" }],
  m_pre_num: [{ validator: validateProps, trigger: "blur" }],
