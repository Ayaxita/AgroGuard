...
const openTransfer = async sheepsinfo => {
  if (sheepsinfo.length !== 0) {
    ...
    ElMessage.warning("请先选择要调整的草地记录！");
  }
};
...
const markSale = async sheepsinfo => {
  if (sheepsinfo.length !== 0) {
                ...
                [
                  h(
                    ElSelect,
                    {
                      ...
                      placeholder: "请填写类型",
                      "onUpdate:modelValue": val => {
                        console.log("Selected type value:", val); // 打印选中的值，用于调试
    ...
    ElMessage.warning("请先选择要标记的草地记录！");
  }
};
...
const generateQRCode = async params => {
  if (params.length === 1) {
    ...
    ElMessage.warning("请先选择要生成二维码的草地记录！");
  } else {
    ElMessage.warning("只能选择一条草地记录生成二维码！");
  }
};
...
const updateMon_age = async () => {
  ElMessageBox.confirm(
    "此操作会根据当前日期更新所有草地生长期并执行分类，耗时较多，请在空闲时操作，你确认要现在更新吗？",
    "温馨提示",
    { type: "warning" }
  ).then(() => {
    updateMonAge();
...
const updateHouse = async () => {
  ElMessageBox.confirm(
    "此操作会根据已经上传的草地数据，对草地区域信息进行更新，耗时较多，请在空闲时操作，你确认要现在更新吗？",
    "温馨提示",
    { type: "warning" }
  ).then(() =>
