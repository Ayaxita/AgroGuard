...
import { getManuList, editManu, addManu, exportSheepInfo, delManu } from "../api/manu";
import { d_healthAb_colorType, d_healthAb_smellType, BooleanType, d_healthTestis_shapeType } from "@/assets/json/typeListJson";
...
const downloadFile = async () => {
  ElMessageBox.confirm("确认导出用户数据?", "温馨提示", { type: "warning" }).then(() =>
    useDownload(exportSheepInfo, "草地监测导出结果", proTable.value?.searchParam)
  );
};
//导出所选数据
const downloadSelectFile = async selectedListIds => {
  ...
  ElMessageBox.confirm(`确认导出所选的${selectedSheepData.length}条草地监测数据?`, "温馨提示", { type: "warning" }).then(() => {
    // 调用useDownload函数进行导出,并传入所选草地监测数据、导出文件名以及其他可能需要的参数(这里假设暂时不需要其他参数)
    useDownload(() => exportSheepInfo(selectedSheepData), `所选草地监测导出结果`, null);
  });
};
// 打开 drawer(新增、查看、编辑)
