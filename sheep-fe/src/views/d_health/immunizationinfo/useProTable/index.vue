...
import { getManuList, editManu, addManu, delImmunizationinfo, exportSheepInfo } from "../api/manu";
import { colonyTransferReasonType, manuScaleType, manutypeType } from "@/assets/json/typeListJson";
...
const downloadFile = async () => {
  ElMessageBox.confirm("确认导出用户数据?", "温馨提示", { type: "warning" }).then(() =>
    useDownload(exportSheepInfo, "草地防护导出结果", proTable.value?.searchParam)
  );
};
//导出所选数据
const downloadSelectFile = async selectedListIds => {
  ...
  ElMessageBox.confirm(`确认导出所选的${selectedSheepData.length}条草地数据?`, "温馨提示", { type: "warning" }).then(() => {
    ...
    useDownload(() => exportSheepInfo(selectedSheepData), `草地防护导出结果`, null);
  });
};
// 打开 drawer(新增、查看、编辑)
