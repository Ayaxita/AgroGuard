  ...
  <div class="table-box">
    ...
    >
      ...
      <template #tableHeader="scope">
        <el-button type="primary" size="small" :icon="CirclePlus" @click="openDrawer('新增')">新增草地记录</el-button>
        <el-button type="primary" size="small" :icon="Check" @click="updatescore()">草地评分</el-button>
        <el-button type="primary" size="small" :icon="Check" @click="updateAscore(scope.selectedListIds)">所选草地评分</el-button>
      ...
      <template #operation="scope">
        ...
        <el-button type="primary" link :icon="Download" @click="downloadSelectFile(scope.row)">下载</el-button>
...
import { useDownload, useDownloadWord } from "@/hooks/useDownload";
...
// 存储选中的棚舍 ID
const selectedHouseName = ref<number | null>(null);
// **动态生成所属棚和栏舍**
...
const hurdleType = computed(() => {
  ...
  //     }))
  //   : [];
  console.log(selectedHouseName.value);

  // if (selectedHouseName.value == null) {
  if (!selectedHouseName.value) {
    // 如果未选择棚舍，显示所有栏舍
    return houses.value
    ...
    // 如果已选择棚舍，只显示该棚舍的栏舍
    const selectedHouse = houses.value.find(house => house.house_name === selectedHouseName.value);
...
const columns = reactive<ColumnProps<User.ResUserList>[]>([
  ...
  { prop: "operation", label: "操作", fixed: "left", width: 250 },
  ...
  {
    prop: "ram_grandfather_ele_num",
    label: "历史编号1",
    width: 150,
    search: { el: "input" }
  },
  {
    prop: "ram_grandfather_pre_num",
    label: "历史编号2",
    width: 115,
    search: { el: "input" }
  },
  {
    prop: "ram_grandmother_ele_num",
    label: "历史编号3",
    width: 150,
    search: { el: "input" }
  },
  {
    prop: "ram_grandmother_pre_num",
    label: "历史编号4",
    width: 115,
    search: { el: "input" }
  },
  {
    prop: "ewe_grandfather_ele_num",
    label: "扩展编号1",
    width: 150,
    search: { el: "input" }
  },
  {
    prop: "ewe_grandfather_pre_num",
    label: "扩展编号2",
    width: 140,
    search: { el: "input" }
  },
  {
    prop: "ewe_grandmother_ele_num",
    label: "扩展编号3",
    width: 150,
    search: { el: "input" }
  },
  {
    prop: "ewe_grandmother_pre_num",
    label: "扩展编号4",
    width: 140,
    search: { el: "input" }
  },
...
const downloadFile = async () => {
  ElMessageBox.confirm("确认导出草地数据?", "温馨提示", { type: "warning" }).then(() =>
    useDownload(exportSheepInfo, "草地导出结果", proTable.value?.searchParam)
  );
};
          ...
          h(
            ElForm,
            {
              ...
              rules: {
                ...
                hurdle_id: [{ required: true, message: "请选择新区段", trigger: "change" }]
              }
            },
            [
              h(
...
const openDetail = async params => {
  console.log(params);
  const href = router.resolve(`/xipu?ele_num=${params.ele_num}`);
  const newWindow = window.open(href.href, "_blank");
  if (newWindow) {
    // 显示加载提示
    newWindow.document.open();
    newWindow.document.write('<div style="text-align: center; padding-top: 50px;">正在加载，请稍候...</div>');
    newWindow.document.close();
  }

  console.log(newWindow);
  // 确保params是完整的HTML字符串
  const result = await openData(params);

  // 尝试将结果转换为字符串
  const htmlData = String(result);

  console.log("我已经获取到网页数据了");

  if (newWindow) {
    newWindow.document.open();
    newWindow.document.write(htmlData); // 类型匹配，不再报错
    newWindow.document.close();
  }
};
// router.push(`/statistic/xipu/detail/index/${params.ele_num}`);
// const href = router.resolve(`/statistic/xipu/detail/index?ele_num=${params.ele_num}`);
// window.open(href.href, "_blank");
//草地评分，所选
...
const updatescore = async () => {
  ElMessageBox.confirm("此操作会对所有正常草地记录进行评分，耗时较多，请在空闲时操作，你确认要现在操作吗？", "温馨提示", {
    type: "warning"
  }).then(() => {
    MakeScore().then(res => {
      if (res.code === 200) {
...
const updateHouse = async () => {
  ElMessageBox.confirm(
    "此操作会根据已经上传的数据，对草地区域信息进行更新，耗时较多，请在空闲时操作，你确认要现在更新吗？",
    "温馨提示",
    { type: "warning" }
  ).then(() =>
