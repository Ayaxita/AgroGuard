...
import { getEwe, searchEweSheep, addBreeding } from "../api/manu";
...
const handleDeliveryDateChange = async (newDate: string) => {
  ...
  try {
    // 调用更新分娩日期的 API
    const response = await searchEweSheep(params);
    // 假设 response.data 符合 ResultData<SheepData> 类型
    // const data = response.data as ResultData<SheepData>;
    ...
    if (data) {
      ...
      ElMessage.success("此记录在配种信息表中有记录，请继续添加");
      // 如果响应成功，填充关联信息编号
