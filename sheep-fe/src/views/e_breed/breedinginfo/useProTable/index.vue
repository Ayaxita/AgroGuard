...
const downloadFile = async () => {
  ElMessageBox.confirm("确认导出用户数据?", "温馨提示", { type: "warning" }).then(() =>
    useDownload(exportSheepInfo, "草地批次信息导出结果", proTable.value?.searchParam)
  );
};
//导出所选数据
const downloadSelectFile = async selectedListIds => {
  ...
  ElMessageBox.confirm(`确认导出所选的${selectedSheepData.length}条数据?`, "温馨提示", { type: "warning" }).then(() => {
    ...
    useDownload(() => exportSheepInfo(selectedSheepData), `草地批次信息导出结果`, null);
  });
};
