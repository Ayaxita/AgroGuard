...
def Commit_calculate_asset_stats(basic_ids):
    ...
    latest_condition_subquery = (
        db.session.query(
            ...
            .filter(BasicBreederconditioninfo.basic_id.in_(basic_ids))
            .group_by(BasicBreederconditioninfo.basic_id)
            .subquery()
    )
    ...
    normal_query = (
        db.session.query(
            ...
            BasicBreederconditioninfo,
            and_(
                BasicBreederconditioninfo.basic_id == latest_condition_subquery.c.basic_id,
                BasicBreederconditioninfo.date == latest_condition_subquery.c.max_date
            )
        )
            .filter(
    ...
    special_query = db.session.query(
        ...
        BasicBasicinfo.purpose == 2,
        BasicBasicinfo.state == 1,
        BasicBasicinfo.id.in_(basic_ids)  # 只处理传入的 basic_id 中哺乳羔羊的记录
    )
...
def export_daily_income():
    ...
    #     info = (
    #         db.session.query(AnalysisDailyIncome)#数据库中查询这些表
    ...
    for info1 in info:
        print(info1)
        data_list.append({
            '种羊销售只数': round(info1.number_0, 2) if info1.number_0 is not None else None,
            '种羊销售额': round(info1.value_0, 2) if info1.value_0 is not None else None,
            '育肥羊销售只数': round(info1.number_1, 2) if info1.number_1 is not None else None,
            '育肥羊销售额': round(info1.value_1, 2) if info1.value_1 is not None else None,
            '哺乳羔羊销售只数': round(info1.number_2, 2) if info1.number_2 is not None else None,
            '哺乳羔羊销售额': round(info1.value_2, 2) if info1.value_2 is not None else None,
            '淘汰羊销售只数': round(info1.number_3, 2) if info1.number_3 is not None else None,
            '淘汰羊销售额': round(info1.value_3, 2) if info1.value_3 is not None else None,
            '其他羊销售只数': round(info1.number_9, 2) if info1.number_9 is not None else None,
            '其他羊销售额': round(info1.value_9, 2) if info1.value_9 is not None else None,
            '羊粪销售额': round(info1.dung_value, 2) if info1.dung_value is not None else None,
            '羊毛销售额': round(info1.wool_value, 2) if info1.wool_value is not None else None,
            '羊皮销售额': round(info1.skin_value, 2) if info1.skin_value is not None else None,
            '有机肥销售额': round(info1.manure_value, 2) if info1.manure_value is not None else None,
            '饲草料销售额': round(info1.feed_value, 2) if info1.feed_value is not None else None,
            '羊副产品销售额': round(info1.producted_value, 2) if info1.producted_value is not None else None,
            '其他销售额': round(info1.other_value, 2) if info1.other_value is not None else None,
            '日期': info1.f_date,
    ...
    filename = os.path.join(os.getcwd(), 'App', 'analysis', 'export_excel', 'income_info.xlsx')
...
def export_daily_sheep_asset():
    ...
    for info1 in info:
        print(info1)
        data_list.append({
            '湖羊种公羊价值': round(info1.hu_0_0, 2) if info1.hu_0_0 is not None else None,
            '湖羊种母羊价值': round(info1.hu_0_1, 2) if info1.hu_0_1 is not None else None,
            '湖羊育肥公羊价值': round(info1.hu_1_0, 2) if info1.hu_1_0 is not None else None,
            '湖羊育肥母羊价值': round(info1.hu_1_1, 2) if info1.hu_1_1 is not None else None,
            '湖羊哺乳公羔羊(1月)价值': round(info1.hu_2_0, 2) if info1.hu_2_0 is not None else None,
            '湖羊哺乳母羔羊(1月)价值': round(info1.hu_2_1, 2) if info1.hu_2_1 is not None else None,
            '湖羊哺乳公羔羊(2月)价值': round(info1.hu_3_0, 2) if info1.hu_3_0 is not None else None,
            '湖羊哺乳母羔羊(2月)价值': round(info1.hu_3_1, 2) if info1.hu_3_1 is not None else None,
            '小尾寒羊种公羊价值': round(info1.xw_0_0, 2) if info1.xw_0_0 is not None else None,
            '小尾寒羊种母羊价值': round(info1.xw_0_1, 2) if info1.xw_0_1 is not None else None,
            '小尾寒羊育肥公羊价值': round(info1.xw_1_0, 2) if info1.xw_1_0 is not None else None,
            '小尾寒羊育肥母羊价值': round(info1.xw_1_1, 2) if info1.xw_1_1 is not None else None,
            '小尾寒羊哺乳公羔羊(1月)价值': round(info1.xw_2_0, 2) if info1.xw_2_0 is not None else None,
            '小尾寒羊哺乳母羔羊(1月)价值': round(info1.xw_2_1, 2) if info1.xw_2_1 is not None else None,
            '小尾寒羊哺乳公羔羊(2月)价值': round(info1.xw_3_0, 2) if info1.xw_3_0 is not None else None,
            '小尾寒羊哺乳母羔羊(2月)价值': round(info1.xw_3_1, 2) if info1.xw_3_1 is not None else None,
            '日期': info1.f_date,
...
@analysis.route('/analysis/daily_report', methods=['POST'])  # 疫苗预警 后端接口
def get_dailyReport():
    pageNum = int(request.json.get('pageNum'))
    pageSize = int(request.json.get('pageSize'))
...
def export_dailyReport():
    ...
    if selected_ids:
        # 如果有传来的所选数据的id列表，导出所选数据
        immunization_info = Analysisdailysheet.query.filter(Analysisdailysheet.id.in_(selected_ids)).all()
    else:
        # 否则，导出所有羊只数据
        immunization_info = Analysisdailysheet.query.all()
    ...
    for info in immunization_info:
        data_list.append({
            '日期': info.date,
            '羊只购买费用': round(info.buysheep_fees, 2) if info.buysheep_fees is not None else None,
    ...
    filename = os.path.join(os.getcwd(), 'App', 'analysis', 'export_excel', 'dailyReport_info.xlsx')
    df.to_excel(filename, index=False)
...
def export_stocksheet():
    ...
    if selected_ids:
        # 如果有传来的所选数据的id列表，导出所选数据
        immunization_info = Analysisdailystocksheet.query.filter(Analysisdailystocksheet.id.in_(selected_ids)).all()
    else:
        # 否则，导出所有羊只数据
        immunization_info = Analysisdailystocksheet.query.all()
    print("_________________________________________")
    # print(immunization_info)
    # basic_info = BasicBasicinfo.query.filter_by(id=data['basic_id']).first()
    ...
    filename = os.path.join(os.getcwd(), 'App', 'analysis', 'export_excel', 'stocksheet_info.xlsx')
    df.to_excel(filename, index=False)
