...
def del_houseinfo():
    ...
    for i in ids:
        ...
        if len(sons) > 0:
            ...
            return jsonify({
                "code": 500,
                "msg": '棚栏下存在栏舍，删除失败，请先删除其下的数据再来操作'
            })
...
def set_hurdle():
    ...
    # 检查每个栏舍是否有羊只，如果有羊只则无法删除
    for house in houses_to_merge:
        if house.sheep_quantity > 0:
            return jsonify({
                "code": 500,
                "msg": '栏舍下存在羊只，合并失败，请先将羊只转圈再来操作'
            })

    # 获取要保留的字段（第一个栏舍的值）
    pid = houses_to_merge[0].pid  # 棚舍按照合并前
    build_time = houses_to_merge[0].build_time  # 创建时间
    function = houses_to_merge[0].function  # 功能
    h_type = houses_to_merge[0].h_type  # 羊舍类型
    staff = houses_to_merge[0].staff  # 管理人员
    f_staff = houses_to_merge[0].f_staff  # 创建人员
    difinfect_time = houses_to_merge[0].difinfect_time  # 消毒时间
    sheep_type = houses_to_merge[0].sheep_type  # 羊只类型
    belong = 0  # 默认值
    sheep_quantity = 0  # 默认值

    # 拼接需要拼接的字段
    name_str = ','.join(house.name for house in houses_to_merge)  # 栏舍名称
    h_lwh_str = ','.join(house.h_lwh for house in houses_to_merge)  # 长宽高
    sports_lwh_str = ','.join(house.sports_lwh for house in houses_to_merge)  # 运动场长宽高
    area_pro_str = ','.join(house.area_pro for house in houses_to_merge)  # 羊只面积比例

    # 获取每个栏舍的 area 字段（假设是浮动类型）
    ...
    for house in houses_to_unseal:
        ...
        for i in range(num_splits):
            ...
            new_house = ColonyHouseinfo(
                ...
                sheep_quantity=house.sheep_quantity,  # 保持不变
                sheep_type=house.sheep_type,  # 保持不变
                name=name_list[i],  # 拆分后的 name
                h_lwh=h_lwh_list[i],  # 拆分后的 h_lwh
                sports_lwh=sports_lwh_list[i],  # 拆分后的 sports_lwh
                area_pro=area_pro_list[i],  # 拆分后的 area_pro
                add_area=None,  # 拆分后的 add_area
...
def del_hurdleinfo():
    # 获取栏舍编号
    ids = request.get_json()
    # 可以一次删除多个栏舍，遍历每个栏舍，判断是否有羊只，有则不能删除
    for i in ids:
        # 获取该栏舍信息
