...
#将死羔信息回显到list的最后一个,在产后编辑和查看的时候用
...
#发情信息
@e_breed.route('/e_breed/rutinfo', methods=['POST'])#获取发情信息
def get_rutinfo():
...
@e_breed.route('/e_breed/rutinfo/del', methods=['POST'])#删除发情信息
def del_rutinfo():
...
@e_breed.route('/e_breed/rutinfo/edit', methods=['POST'])#编辑发情信息
def edit_rutinfo():
...
@e_breed.route('/e_breed/rutinfo/add', methods=['POST'])#添加发情信息
def add_rutinfo():
    data = request.get_json()
    #去除剂量两端的空格
    # if 'dose' in data:
...
#  母羊发情
#获取月龄8以上的健康未销售的母羊记录(在产后是12月，在发情和配种时8月).。。。。。。。。。。。。。改成6月了
...
def export_rutinfo():
    ...
    if selected_ids:
        ...
        rut_info = (
            db.session.query(EBreedRutinfo, BasicBasicinfo.ele_num, BasicBasicinfo.pre_num)#数据库中查询这些表
                .join(BasicBasicinfo, BasicBasicinfo.id == EBreedRutinfo.basic_id)
    ...
    for info,ele_num,pre_num in rut_info:
        breeding_value = breeding_dict.get(info.breeding, info.breeding)  # 通过字典获取对应中文值，若不存在则使用原数据
        print(info)
        data_list.append({
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'rut_info.xlsx')
...
def export_semencollectinfo():
    ...
    if selected_ids:
        # 如果有传来的所选数据的id列表，导出所选数据
        semencollect_info = (
            db.session.query(EBreedSemencollectinfo, BasicBasicinfo.ele_num, BasicBasicinfo.pre_num)#数据库中查询这些表
                .join(BasicBasicinfo, BasicBasicinfo.id == EBreedSemencollectinfo.basic_id)
                .filter(EBreedSemencollectinfo.id.in_(selected_ids))
                .all()
        )
        print(semencollect_info)
    else:
        # 否则，导出所有羊只数据
        semencollect_info = (
    ...
    for info,ele_num,pre_num in semencollect_info:
        disused_value = disused_dict.get(info.disused, info.disused)  # 通过字典获取对应中文值，若不存在则使用原数据
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'semencollect_info.xlsx')
    df.to_excel(filename, index=False)

    return send_file(export_path, as_attachment=True)

# 公羊采精
#获取月龄8以上的健康未销售的公羊记录(在产后是12月，在发情和配种时8月)、、、、、、、、、、、、、、、、、、改成6月了
...
def judge_inbreed():
    ...
    参数:
        ram_ele_num (str): 公羊的电子耳号
        ewe_ele_num (str): 母羊的电子耳号
    ...
    try:
        ...
        if not ram_record:
            return jsonify({"code": 404, "msg": f"未找到对应的公羊记录：{ram_ele_num}"})

        # 查询母羊记录
        ewe_record = BasicBasicinfo.query.filter_by(ele_num=ewe_ele_num).first()
        if not ewe_record:
            return jsonify({"code": 404, "msg": f"未找到对应的母羊记录：{ewe_ele_num}"})
...
def export_breedinginfo():
    ...
    # 定义母羊和公羊的别名
    # 在同一查询中对表BasicBasicinfo进行多次连接并避免冲突。
    ewe_info = aliased(BasicBasicinfo)  # 母羊信息的别名
    ram_info = aliased(BasicBasicinfo)  # 公羊信息的别名
    ...
    if selected_ids:
        ...
        breeding_info = (
            db.session.query(
                EBreedBreedinginfo,
                # 从母羊信息中查询电子耳号和防疫耳号，并设置别名
                ewe_info.ele_num.label("ewe_ele_num"),  # 母羊电子耳号     # 在同一查询中对表BasicBasicinfo进行多次连接并避免冲突。
                ewe_info.pre_num.label("ewe_pre_num"),  # 母羊防疫耳号
                # 从公羊信息中查询电子耳号和防疫耳号，并设置别名
                ram_info.ele_num.label("ram_ele_num"),  # 公羊电子耳号
                ram_info.pre_num.label("ram_pre_num")  # 公羊防疫耳号
            )
                # 使用别名 `ewe_info` 和 `ram_info` 分别表示母羊和公羊的基本信息
                .join(ewe_info, ewe_info.id == EBreedBreedinginfo.ewe_id)  # 母羊信息关联
                .join(ram_info, ram_info.id == EBreedBreedinginfo.ram_id)  # 公羊信息关联
                .filter(EBreedBreedinginfo.id.in_(selected_ids))  # 筛选出所选的繁育信息
                .all()
        )
        print(breeding_info)
    else:
        ...
        breeding_info = (
            db.session.query(
                EBreedBreedinginfo,
                # 从母羊信息中查询电子耳号和防疫耳号，并设置别名
                ewe_info.ele_num.label("ewe_ele_num"),  # 母羊电子耳号
                ewe_info.pre_num.label("ewe_pre_num"),  # 母羊防疫耳号
                # 从公羊信息中查询电子耳号和防疫耳号，并设置别名
                ram_info.ele_num.label("ram_ele_num"),  # 公羊电子耳号
                ram_info.pre_num.label("ram_pre_num")  # 公羊防疫耳号
    ...
    for info,ewe_ele_num, ewe_pre_num, ram_ele_num, ram_pre_num in breeding_info:
        Breeding_wayType_value = Breeding_wayType.get(info.breeding_way, info.breeding_way)  # 通过字典获取对应中文值，若不存在则使用原数据
        ...
        # '羊基本信息id':info.basic_id,
        '母羊电子耳号': ewe_ele_num,  # 母羊的电子耳号
        '母羊防疫耳号': ewe_pre_num,  # 母羊的防疫耳号
        '母羊品种': ewe_varietyType_value,
        '公羊电子耳号': ram_ele_num,  # 公羊的电子耳号
        '公羊防疫耳号': ram_pre_num,  # 公羊的防疫耳号
        '公羊品种': ram_varietyType_value,
        '配种日期':info.breeding_date,
        '预产日期':info.pre_production_date,
        '配种方式': Breeding_wayType_value,
        '配种状态': info.breeding_state,
        '配种情期（天）': info.mat_period,
        '单次配种成功率（%）': info.single_ok,
        '操作师': info.staff,
        '创建时间':info.f_date,
        '创建人员':info.f_staff
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'breedinginfo.xlsx')
    df.to_excel(filename, index=False)
...
def export_postnatalinfo():
    ...
    if selected_ids:
        ...
        postnatal_info = (
            db.session.query(
                ...
                # 使用别名 `ewe_info` 和 `ram_info` 分别表示母羊和公羊的基本信息
                .join(EBreedBreedinginfo, EBreedBreedinginfo.id == EBreedPostnatalinfo.breeding_id)
                .join(ewe_info, ewe_info.id == EBreedBreedinginfo.ewe_id)  # 母羊信息关联
                .join(ram_info, ram_info.id == EBreedBreedinginfo.ram_id)  # 公羊信息关联
                .filter(EBreedPostnatalinfo.id.in_(selected_ids))  # 筛选出所选的繁育信息
                .all()
        )
        print(postnatal_info)
    else:
        # 否则，导出所有羊只数据
        breeding_info = (
            db.session.query(
    ...
    for info,ewe_ele_num, ewe_pre_num, ram_ele_num, ram_pre_num in postnatal_info:
        Ewe_healthType_value = Ewe_healthType.get(info.ewe_health, info.ewe_health)  # 通过字典获取对应中文值，若不存在则使用原数据
        ...
        '公羊防疫耳号': ram_pre_num,  # 公羊的防疫耳号
        '配种日期':info.breeding_date,
        '分娩日期':info.delivery_date,
        '母羊产羔率': info.Booroola,
        '母羊健康情况': Ewe_healthType_value,
        '母性情况': Ewe_conditionType_value,
        '产羔数': info.live_num,
        '接生人员': info.birth_attendants,
        '创建时间':info.f_date,
        '创建人员':info.f_staff
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'postnatal_info.xlsx')
    df.to_excel(filename, index=False)

    return send_file(export_path, as_attachment=True)

#获取月龄12以上的健康未销售的母羊记录(在产后是12月，在发情和配种时8月)，，，，，，，，，改成6月和11月了
...
#获取月龄12以上的健康未销售的公羊记录(在产后是12月，在发情和配种时8月)，，，，改成6月和11月了
...
@e_breed.route('/e_breed/postnatalinfo/getlamb', methods=['POST'])#回显产后羔羊
def get_lamb():
...
# 查询母羊信息记录是否已经在配种信息表中有记录（选择完分娩日期后进入）。。。。。。。。改成查7个月之内得了
...
def search_ewe_sheep():
    ...
    # 查询 EBreedBreedinginfo 表，查找对应 ewe_id 并且 breeding_date 大于等于 pre_date（分娩往前推六个月，一只羊从配种到分娩大约150天） 的记录
    ...
    if postnatal_check:
        return jsonify({"code": 400, "msg": "在对应配种记录之后已有产后记录，请勿重复添加产后信息"})
    ...
    if breeding_info:
        ...
        result = {
            ...
            "msg": '成功',
            "data": {
                "ram_ele_num": ram_ele_num   # 返回公羊的 ele_num
            },
            # 如果需要分页或其他数据，可以在这里添加
        }
    else:
...
#添加配种信息表里母羊与公羊信息到配种信息表（在配种信息表中生成一条新的记录），，，，，，，改成分娩日期往前推150天了
...
#添加产后信息（同时添加一条孕检信息记录），，，，改成分娩日期往前推150天了
...
def add_postnatalinfo():
    ...
    if breeding_info:
        ...
        return jsonify({"code": 404, "msg": "未找到符合条件的配种记录"}), 404


    # 其实下面这些breeding_date好像都不用写，直接拿就行，因为在上面添加配种信息步骤里已经弄了，但是我忘了这块是不是这么写的逻辑了，所以不删了
    ...
    for i in range(1, int(data['live_num']) + 1):  # 根据 live_num 确定需要处理的羊羔数量
        ...
        # 获取父母耳号
        if ewe:
            m_pre_num = ewe.pre_num  # 母羊的防疫耳号
            m_ele_num = ewe.ele_num  # 母羊耳号
            variety = ewe.variety
        else:
            m_pre_num = None
            m_ele_num = None
            variety = None
            # 可以添加异常处理或返回错误
            print(f"未找到母羊（ewe_id: {data['ewe_id']}）的防疫耳号")

        if ram:
            f_pre_num = ram.pre_num# 父羊的防疫耳号
            f_ele_num = ram.ele_num
        else:
            f_pre_num = None
            f_ele_num = None
            # 可以添加异常处理或返回错误
            print(f"未找到父羊（ram_id: {data['ram_id']}）的防疫耳号")
        lamb = {
            'breeding_id': data['breeding_id'],
            'tobasic': 0,
            'logo': logo,
            ...
            'birth':delivery_date,
            'bir_weight': data.get(f'bir_weight_{i}'),
            'mother_id': data['ewe_id'],  # 关联母羊的 ewe_id
        ...
        if value is not None:  # 检查值不为 None
            if param == 'f_date':  # 日期需要转换
                ...
                basic_id = BasicBasicinfo.query.filter(
                        BasicBasicinfo.ele_num.like(f'%{value}%')).first().id
                print(basic_id)
                breeding_info = EBreedBreedinginfo.query.filter_by(ewe_id=basic_id).first()#然后再从配种信息表里找到这条配种记录
                print('------------------------')
                print(breeding_info)
                if breeding_info:
                    breeding_id = breeding_info.id
                    print(breeding_id)
                    pregnant_info = EBreedPregnantinfo.query.filter_by(breeding_id=breeding_id).first()#通过配种信息表里的id获取孕检表的id，然后对比孕检表中有没有这条breeding_id，把这个作为搜索条件
...
def export_pregnantinfo():
    ...
    if selected_ids:
        ...
        pregnant_info = (
            db.session.query(EBreedPregnantinfo, BasicBasicinfo.ele_num, BasicBasicinfo.pre_num)
                # 先通过EBreedPregnantinfo的breeding_id关联EBreedBreedinginfo表
                .join(EBreedBreedinginfo, EBreedBreedinginfo.id == EBreedPregnantinfo.breeding_id)
                # 再通过EBreedBreedinginfo的ewe_id关联BasicBasicinfo表
                .join(BasicBasicinfo, BasicBasicinfo.id == EBreedBreedinginfo.ewe_id)
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'pregnant_info.xlsx')
    df.to_excel(filename, index=False)
...
def export_artificialfeedinginfo():
    ...
    if selected_ids:
        ...
        artificialfeeding_info = (
            db.session.query(EBreedArtificialfeedinginfo, EBreedLambinfo.ele_num, EBreedLambinfo.pre_num)#数据库中查询这些表
                .join(EBreedLambinfo, EBreedLambinfo.id == EBreedArtificialfeedinginfo.lamb_id)
                .filter(EBreedArtificialfeedinginfo.id.in_(selected_ids))
                .all()
        )
        print(artificialfeeding_info)
    else:
        # 否则，导出所有羊只数据
        artificialfeeding_info = (
            db.session.query(EBreedArtificialfeedinginfo, EBreedLambinfo.ele_num, EBreedLambinfo.pre_num)#数据库中查询这些表
    ...
    for info,ele_num,pre_num in artificialfeeding_info:
        # Feeding_wayType_value = Feeding_wayType.get(info.feeding_way, info.feeding_way)  # 通过字典获取对应中文值，若不存在则使用原数据
        ...
        '羔羊电子耳号': ele_num,
        '羔羊防疫耳号': pre_num,
        '繁殖日期':info.delivery_date,
        '体重监测':info.BW,
        '人工喂养原因':info.reason,
        '喂养物': info.feeding_material,
        '食量': info.mcal,
        '健康情况': info.health,
        '求助情况': info.help,
        '用量': info.dose,
        '喂养人员': info.feeding_staff,
        '创建人员':info.f_staff
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'artificialfeeding_info.xlsx')
    df.to_excel(filename, index=False)
...
def export_weaninginfo():
    ...
    if selected_ids:
        # 如果有传来的所选数据的id列表，导出所选数据
        weaning_info = (
            db.session.query(EBreedWeaninginfo, EBreedLambinfo.ele_num, EBreedLambinfo.pre_num)#数据库中查询这些表
                .join(EBreedLambinfo, EBreedLambinfo.id == EBreedWeaninginfo.lamb_id)
    ...
    for info,ele_num,pre_num in weaning_info:
        ...
        '羔羊电子耳号': ele_num,
        '羔羊防疫耳号': pre_num,
        '断奶日期':info.Delivery_date,
        '喂养方式':Feeding_wayType_value,
        '出生重': info.Bir_weight,
        '断奶重（出栏体重）': info.wea_weight,
        '断奶评级':WeaningRankType_value,
        '创建人员':info.f_staff
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'weaning_info.xlsx')
    df.to_excel(filename, index=False)
...
def export_lambinfo():
    ...
    if selected_ids:
        ...
        lamb_info = (
            db.session.query(EBreedLambinfo)
                .filter(EBreedLambinfo.id.in_(selected_ids))  # 筛选出所选的繁育信息
                .all()
        )
        print(lamb_info)
    else:
    ...
    for info in lamb_info:
        tobasic_dict_value = tobasic_dict.get(info.tobasic, info.tobasic)  # 通过字典获取对应中文值，若不存在则使用原数据
        ...
        '品种': varietyType_value,
        '性别': sexTypee_value,
        '产地名': info.manu_info_name,
        '状态': Lamb_statTypee_value,
        '出生重（Kg）': info.bir_weight,
        '断奶重（Kg）': info.wea_weight,
        '圈舍名称': info.house_name,
        '圈栏名称': info.hurdle_name,
        '月龄': info.mon_age,
        '毛色': colorType_value,
        '外貌等级': rankType_value,
        '母羊电子耳号': info.m_ele_num,  # 母羊的电子耳号
        '母羊防疫耳号': info.m_pre_num,  # 母羊的防疫耳号
        '公羊电子耳号': info.f_ele_num,  # 公羊的电子耳号
        '公羊防疫耳号': info.f_pre_num,  # 公羊的防疫耳号
        '基因型': gene_aType_value,
        '总分数': info.score,
        '创建时间':info.f_date,
        '创建人员':info.f_staff,
        '备注': info.note
    ...
    filename = os.path.join(os.getcwd(), 'App', 'e_breed', 'export_excel', 'lamb_info.xlsx')
    df.to_excel(filename, index=False)

    return send_file(export_path, as_attachment=True)

#添加入棚信息tobasic添加记录到基本信息表并修改羔羊信息表
...
def edit_and_tobasic():
    ...
    try:
        ...
        print(lamb_info)
        if not lamb_info:
            return jsonify({"code": 404, "msg": "羔羊信息未找到"})
