...
def edit_basic_info():
    ...
    # 在修改基本信息表时，更新断奶信息到羔羊表和断奶信息表
    #这里防止报错加了一个逻辑，先判断有咩有羔羊和断奶信息（）因为外购肯定没有，所以很有必要加这个判断
    birth = data['birth']
...
def export_basic_info():
    ...
    for info in basic_infos:
        ...
        list.append({
            ...
            '毛色': color_value,
            '性别': sex_value,
            '出生日期': info.birth.isoformat() if info.birth else None,
            '月龄': info.mon_age,
            '原产地': info.manu_info_name,
            '状态': state_value,
            '所属棚': info.house_name,
            '栏': info.hurdle_name,
            '出生体重': info.bir_weight,
            '断奶重': info.wea_weight,
            '等级': rank_value,
            '父电子耳号': info.f_ele_num,
            '父防疫耳号': info.f_pre_num,
            '母电子耳号': info.m_ele_num,
            '母防疫耳号': info.m_pre_num,
            '添加人': info.f_staff,
            ...
            '2月龄评分评': info.score_2,
            '6月龄评分': info.score_6,
            '12月龄评分': info.score_12,
            '24月龄评分': info.score_24
        })
...
def import_template():
    filename = os.path.join(os.getcwd(), 'App', 'basic', 'temp_excel', '羊只基本信息导入模板.xls')
...
@Statistic.route('/statistic/makescore', methods=['POST'])     #羊只评分标准
...
@Statistic.route('/statistic/makescore/edit', methods=['POST'])   #修改羊只评分标准
def edit_makescore():
    data = request.get_json()
    print("--data-->", data)
    id = data['id']
...
@Statistic.route('/statistic/xipu/scoremake', methods=['POST'])     #羊只打分逻辑
def scoremake(basic_id):
        ...
        if basic:
            ...
            if lamb_self:  # 如果羔羊信息表中存在该basic_id
                breed_id = lamb_self.breeding_id  # 获取配种信息id
                postna = db.session.query(EBreedPostnatalinfo).filter(
                    EBreedPostnatalinfo.breeding_id == breed_id).first()  # 获取self的产后信息
                ...
                if postna:
                    together_num = postna.live_num  # 获取self的产羔数
        score_all = 0
        basic_score = 0
        benshen_score = 0
        houyi_score = 0
...
def xiazai_xipu():
        ...
        if not selected_sheep:
            return jsonify({"error": "未接收到有效的羊只数据"}), 400
        print('我进来了')
        try:
