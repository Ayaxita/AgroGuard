...
def export_daily_income():
    try:
        ...
        for info1 in info:
            data_list.append({
                '草地销售-种苗销售-条数': round(info1.number_0, 2) if info1.number_0 is not None else None,
                '草地销售-种苗销售-销售额(元)': round(info1.value_0, 2) if info1.value_0 is not None else None,
                '草地销售-生长期销售-条数': round(info1.number_1, 2) if info1.number_1 is not None else None,
                '草地销售-生长期销售-销售额(元)': round(info1.value_1, 2) if info1.value_1 is not None else None,
                '草地销售-幼苗批次销售-条数': round(info1.number_2, 2) if info1.number_2 is not None else None,
                '草地销售-幼苗批次销售-销售额(元)': round(info1.value_2, 2) if info1.value_2 is not None else None,
                '草地销售-淘汰地块销售-条数': round(info1.number_3, 2) if info1.number_3 is not None else None,
                '草地销售-淘汰地块销售-销售额(元)': round(info1.value_3, 2) if info1.value_3 is not None else None,
                '草地销售-其他-条数': round(info1.number_9, 2) if info1.number_9 is not None else None,
                '草地销售-其他-销售额(元)': round(info1.value_9, 2) if info1.value_9 is not None else None,
                '草地副产品销售-草地残渣-销售额(元)': round(info1.dung_value, 2) if info1.dung_value is not None else None,
                '草地副产品销售-采收产物-销售额(元)': round(info1.wool_value, 2) if info1.wool_value is not None else None,
                '草地副产品销售-地表覆盖物-销售额(元)': round(info1.skin_value, 2) if info1.skin_value is not None else None,
                '草地副产品销售-有机肥-销售额(元)': round(info1.manure_value, 2) if info1.manure_value is not None else None,
                '草地副产品销售-草地投入物-销售额(元)': round(info1.feed_value, 2) if info1.feed_value is not None else None,
                '草地副产品销售-草地副产物-销售额(元)': round(info1.producted_value, 2) if info1.producted_value is not None else None,
                '草地副产品销售-其他-销售额(元)': round(info1.other_value, 2) if info1.other_value is not None else None,
...
def export_daily_grass_asset():
    try:
        ...
        for info1 in info:
            data_list.append({
                '经济型草地-种苗-A组-价值(元)': round(info1.hu_0_0, 2) if info1.hu_0_0 is not None else None,
                '经济型草地-种苗-B组-价值(元)': round(info1.hu_0_1, 2) if info1.hu_0_1 is not None else None,
                '经济型草地-生长期草地-A组-价值(元)': round(info1.hu_1_0, 2) if info1.hu_1_0 is not None else None,
                '经济型草地-生长期草地-B组-价值(元)': round(info1.hu_1_1, 2) if info1.hu_1_1 is not None else None,
                '经济型草地-幼苗批次(1月)-A组-价值(元)': round(info1.hu_2_0, 2) if info1.hu_2_0 is not None else None,
                '经济型草地-幼苗批次(1月)-B组-价值(元)': round(info1.hu_2_1, 2) if info1.hu_2_1 is not None else None,
                '经济型草地-幼苗批次(2月)-A组-价值(元)': round(info1.hu_3_0, 2) if info1.hu_3_0 is not None else None,
                '经济型草地-幼苗批次(2月)-B组-价值(元)': round(info1.hu_3_1, 2) if info1.hu_3_1 is not None else None,
                '混合型草地-种苗-A组-价值(元)': round(info1.xw_0_0, 2) if info1.xw_0_0 is not None else None,
                '混合型草地-种苗-B组-价值(元)': round(info1.xw_0_1, 2) if info1.xw_0_1 is not None else None,
                '混合型草地-生长期草地-A组-价值(元)': round(info1.xw_1_0, 2) if info1.xw_1_0 is not None else None,
                '混合型草地-生长期草地-B组-价值(元)': round(info1.xw_1_1, 2) if info1.xw_1_1 is not None else None,
                '混合型草地-幼苗批次(1月)-A组-价值(元)': round(info1.xw_2_0, 2) if info1.xw_2_0 is not None else None,
                '混合型草地-幼苗批次(1月)-B组-价值(元)': round(info1.xw_2_1, 2) if info1.xw_2_1 is not None else None,
                '混合型草地-幼苗批次(2月)-A组-价值(元)': round(info1.xw_3_0, 2) if info1.xw_3_0 is not None else None,
                '混合型草地-幼苗批次(2月)-B组-价值(元)': round(info1.xw_3_1, 2) if info1.xw_3_1 is not None else None,
                '记录日期': info1.f_date.isoformat() if info1.f_date else None,
            })
        df = pd.DataFrame(data_list)
...
def export_dailyReport():
    try:
        ...
        for info in daily_report_info:
            data_list.append({
                '记录日期': info.date.isoformat() if info.date else None,
                '种苗购买费用': round(info.buygrass_fees, 2) if info.buygrass_fees is not None else None,
                '草地投入费用': round(info.caoliao_fees, 2) if info.caoliao_fees is not None else None,
                '精准投入费用': round(info.jingliao_fees, 2) if info.jingliao_fees is not None else None,
                '防护费用': round(info.yimiao_fees, 2) if info.yimiao_fees is not None else None,
                '药物费用': round(info.yaopin_fees, 2) if info.yaopin_fees is not None else None,
                '监测检验费用': round(info.test_fees, 2) if info.test_fees is not None else None,
                ...
                '地租费用': round(info.land_fees, 2) if info.land_fees is not None else None,
                '设备维修费用': round(info.maintenance_fees, 2) if info.maintenance_fees is not None else None,
                ...
                '研发费用': round(info.research_fees, 2) if info.research_fees is not None else None,
                '其他费用': round(info.other_fees, 2) if info.other_fees is not None else None,
                '其他费用说明': info.other_text,
...
def export_stocksheet():
    try:
        ...
        for info in stocksheet_info:
            data_list.append({
                '记录日期': info.date.isoformat() if info.date else None,
                '草地投入-残渣类投入-数量': round(info.garlicskin_num, 2) if info.garlicskin_num is not None else None,
                '草地投入-残渣类投入-总价值': round(info.garlicskin_val, 2) if info.garlicskin_val is not None else None,
                '草地投入-秸秆类投入-数量': round(info.peanutseedling_num, 2) if info.peanutseedling_num is not None else None,
                '草地投入-秸秆类投入-总价值': round(info.peanutseedling_val, 2) if info.peanutseedling_val is not None else None,
                '草地投入-青贮饲料-数量': round(info.ensilage_num, 2) if info.ensilage_num is not None else None,
                '草地投入-青贮饲料-总价值': round(info.ensilage_val, 2) if info.ensilage_val is not None else None,
                '草地投入-其他投入-数量': round(info.otherforage_num, 2) if info.otherforage_num is not None else None,
                '草地投入-其他投入-总价值': round(info.otherforage_val, 2) if info.otherforage_val is not None else None,
                '精准投入-谷物类投入-数量': round(info.corn_num, 2) if info.corn_num is not None else None,
                '精准投入-谷物类投入-总价值': round(info.corn_val, 2) if info.corn_val is not None else None,
                '精准投入-复合投入-数量': round(info.premix_num, 2) if info.premix_num is not None else None,
                '精准投入-复合投入-总价值': round(info.premix_val, 2) if info.premix_val is not None else None,
                '精准投入-纤维类投入-数量': round(info.bran_num, 2) if info.bran_num is not None else None,
                '精准投入-纤维类投入-总价值': round(info.bran_val, 2) if info.bran_val is not None else None,
                '精准投入-豆类投入-数量': round(info.soybeanmeal_num, 2) if info.soybeanmeal_num is not None else None,
                '精准投入-豆类投入-总价值': round(info.soybeanmeal_val, 2) if info.soybeanmeal_val is not None else None,
                '精准投入-矿物质投入-数量': round(info.salt_num, 2) if info.salt_num is not None else None,
                '精准投入-矿物质投入-总价值': round(info.salt_val, 2) if info.salt_val is not None else None,
                '精准投入-调节剂投入-数量': round(info.bakingsoda_num, 2) if info.bakingsoda_num is not None else None,
                '精准投入-调节剂投入-总价值': round(info.bakingsoda_val, 2) if info.bakingsoda_val is not None else None,
                '精准投入-乳酸钙-数量': round(info.calciumlactate_num, 2) if info.calciumlactate_num is not None else None,
                '精准投入-乳酸钙-总价值': round(info.calciumlactate_val, 2) if info.calciumlactate_val is not None else None,
                '精准投入-其他精准投入-数量': round(info.otherfinefodder_num, 2) if info.otherfinefodder_num is not None else None,
                '精准投入-其他精准投入-总价值': round(info.otherfinefodder_val, 2) if info.otherfinefodder_val is not None else None,
                '防护剂-植保药剂-数量': round(info.smallvaccine_num, 2) if info.smallvaccine_num is not None else None,
                '防护剂-植保药剂-总价值': round(info.smallvaccine_val, 2) if info.smallvaccine_val is not None else None,
                '防护剂-综合防护剂-数量': round(info.threePfourD_num, 2) if info.threePfourD_num is not None else None,
                '防护剂-综合防护剂-总价值': round(info.threePfourD_val, 2) if info.threePfourD_val is not None else None,
                '防护剂-真菌病防护-数量': round(info.footAmouthdisease_num, 2) if info.footAmouthdisease_num is not None else None,
                '防护剂-真菌病防护-总价值': round(info.footAmouthdisease_val, 2) if info.footAmouthdisease_val is not None else None,
                '防护剂-多联必应-数量': round(info.duolianbiying_num, 2) if info.duolianbiying_num is not None else None,
                '防护剂-多联必应-总价值': round(info.duolianbiying_val, 2) if info.duolianbiying_val is not None else None,
                '防护剂-其他防护剂-数量': round(info.othervaccine_num, 2) if info.othervaccine_num is not None else None,
                '防护剂-其他防护剂-总价值': round(info.othervaccine_val, 2) if info.othervaccine_val is not None else None,
                '防护药品-庆大霉素-数量': round(info.gentamicin_num, 2) if info.gentamicin_num is not None else None,
                '防护药品-庆大霉素-总价值': round(info.gentamicin_val, 2) if info.gentamicin_val is not None else None,
                '防护药品-中灵-数量': round(info.zhongling_num, 2) if info.zhongling_num is not None else None,
                '防护药品-中灵-总价值': round(info.zhongling_val, 2) if info.zhongling_val is not None else None,
                '防护药品-替米考星-数量': round(info.tilmicosin_num, 2) if info.tilmicosin_num is not None else None,
                '防护药品-替米考星-总价值': round(info.tilmicosin_val, 2) if info.tilmicosin_val is not None else None,
                '防护药品-其他防护药品-数量': round(info.othermedicine_num, 2) if info.othermedicine_num is not None else None,
                '防护药品-其他防护药品-总价值': round(info.othermedicine_val, 2) if info.othermedicine_val is not None else None
            })
